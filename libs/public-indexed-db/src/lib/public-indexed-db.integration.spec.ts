...
import type { OfflineAttendanceQueueItem, OfflineTotpSeedRecord } from './public-data-schema';
import { normalizeOfflineAttendanceQueueOwnership, PublicDataDatabase } from './public-data-schema';
...
describe('offline public data access integration', () => {
  ...
  beforeEach(async () => {
    ...
    database = new PublicDataDatabase();
  ...
  it('persists calendar events, expires old rows, and records the refresh timestamp', async () => {
    const service = injectService(CalendarDataCacheService);
  ...
  it('returns safe fallback values when offline storage is unavailable', async () => {
    ...
    try {
      ...
      const calendarData = runInInjectionContext(unavailableInjector, () => new CalendarDataCacheService());
  ...
  it('keeps the database unavailable on the server and memoizes it in the browser', () => {
    ...
    try {
      const serverProvider = runInInjectionContext(serverInjector, () => new PublicDatabaseProvider());
      const browserProvider = runInInjectionContext(browserInjector, () => new PublicDatabaseProvider());
