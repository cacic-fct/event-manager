...
// Keep the legacy database name so existing installations continue upgrading
// the same IndexedDB database after the library's broader purpose was renamed.
const PUBLIC_DATA_DATABASE_NAME = 'cacic-public-offline-data';
...
export interface FeatureFlagCacheRecord {
  key: string;
  updatedAt: number;
  value: unknown;
}
...
export class PublicDataDatabase extends Dexie {
  ...
  calendarPreferences!: Table<CalendarPreferencesRecord, string>;
