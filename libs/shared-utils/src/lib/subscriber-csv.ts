...
export type SubscriberCsvField = (typeof SUBSCRIBER_CSV_FIELDS)[number];
...
export interface SubscriberCsvPerson {
  name: string;
  email?: string | null;
  phone?: string | null;
...
export interface SubscriberCsvRecord {
  person?: SubscriberCsvPerson | null;
}
...
export function buildSubscriberCsv(records: SubscriberCsvRecord[], options: SubscriberCsvExportOptions): string {
  ...
  return [headers, ...rows].map((row) => serializeSubscriberCsvRow(row)).join('\r\n');
}
...
export function getSubscriberFieldValue(
  person: SubscriberCsvPerson | null | undefined,
...
function serializeSubscriberCsvRow(values: string[]): string {
