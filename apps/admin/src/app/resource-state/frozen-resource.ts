...
export function isFrozenFromDates(dates: Array<string | Date | null | undefined>, now = new Date()): boolean {
  const validDates = dates.map((date) => toDate(date)).filter((date): date is Date => date !== null);
...
export function isFrozenMajorEvent(majorEvent: Pick<MajorEvent, 'createdAt' | 'endDate'> | null | undefined): boolean {
...
export function isFrozenEventGroup(
  ...
  return isFrozenFromDates([eventGroup.createdAt, ...events.flatMap((event) => [event.createdAt, event.endDate])], now);
}
