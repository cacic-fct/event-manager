...
export class MajorEventsService {
  ...
  async saveMajorEvent(action: CreationPublicationAction = 'DRAFT'): Promise<void> {
    ...
    try {
      ...
      if (action === 'PUBLISH') {
        ...
        this.snackbar.open(action === 'SCHEDULE' ? 'Grande evento salvo como rascunho.' : 'Rascunho salvo.', 'Fechar', {
          duration: 2500,
        });
      }
  ...
  private buildMajorEventPayload(options: { allowIncompleteDraft?: boolean } = {}): MajorEventInput {
    const raw = this.majorEventForm.getRawValue();
    const dates = this.resolveMajorEventDates(raw.startDate, raw.endDate, options.allowIncompleteDraft === true);
  ...
  private resetPriceTiers(groups: ReturnType<MajorEventsService['createPriceTierGroup']>[]): void {
  ...
  private syncPriceTierControls(type: PriceType): void {
    if (type === 'SINGLE') {
      ...
      const tier = this.priceTiers.at(0) as ReturnType<MajorEventsService['createPriceTierGroup']> | null;
