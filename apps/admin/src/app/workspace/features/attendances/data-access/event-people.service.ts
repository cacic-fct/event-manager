...
export interface EventPersonLink {
  personId: string;
  name: string;
}
...
export class EventPeopleService {
  ...
  async listLecturers(eventId: string): Promise<EventPersonLink[]> {
  ...
  async listAttendanceCollectors(eventId: string): Promise<EventPersonLink[]> {
  ...
  private toPersonLink(record: EventLecturer | EventAttendanceCollector): EventPersonLink {
    return {
