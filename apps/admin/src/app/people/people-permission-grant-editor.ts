...
export abstract class PeoplePermissionGrantEditor extends PeopleRecords {
  ...
  applySelectedPermissionPreset(): void {
    const selectedPerson = this.selectedPerson();
    const userId = this.getPersonUserId(selectedPerson);
    const preset = EVENT_MANAGER_PERMISSION_PRESETS.find(
      (item) => item.id === this.permissionGrantForm.controls.presetId.value,
    );
  ...
  getPermissionGrantTargetMetadataLabel(target: EventManagerPermissionGrantTarget): string {
    return [
      getPermissionGrantTargetDateLabel(target, this.permissionGrantScope(), this.locale),
  ...
  private stagePermissionGrantInputs(inputs: EventManagerPermissionGrantInput[], sourceLabel: string): void {
    ...
    for (const input of inputs) {
      const draft = buildGrantDraft(
        input,
        sourceLabel,
        getGrantInputTargetLabel(input, {
