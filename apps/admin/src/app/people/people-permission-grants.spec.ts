...
describe('workspace people permission grant helpers', () => {
  ...
  it('forces global scope for permissions that cannot be scoped to a target', () => {
    expect(
      buildPermissionGrantTargetInput(Permission.Person.Delete, EventManagerPermissionGrantScope.Event, 'event-1'),
    ).toEqual({
  ...
  it('builds stable target keys and labels for scoped grants', () => {
    ...
    expect(getPermissionGrantInputTargetKey(input)).toBe('user-1|event-attendance#collect|EVENT|event-1||');
    expect(
  ...
  it('formats labels, validity windows, target dates, and status from grant metadata', () => {
    ...
    expect(getPermissionGrantSelectionLabel([Permission.EventAttendance.Collect])).toBe('Presenças · Coletar');
    ...
    expect(getPermissionGrantStatusLabel(permissionGrant({ validFrom: '2026-07-02T12:00:00.000Z' }))).toBe('Agendada');
    expect(getPermissionGrantStatusLabel(permissionGrant({ validUntil: '2026-06-30T12:00:00.000Z' }))).toBe('Expirada');
