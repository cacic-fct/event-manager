...
export const PERMISSION_GRANT_GROUPS: PermissionGrantGroup[] = formatPermissionGroups(
  EVENT_MANAGER_PERMISSION_CATALOG,
).map((group) => ({
  resource: group.type,
  label: group.label,
...
export function buildPermissionGrantTargetInput(
  ...
  const effectiveScope = requiresGlobalPermissionGrantScope(permission)
...
export function buildPermissionGrantInputFromDraft(draft: PermissionGrantDraft): EventManagerPermissionGrantInput {
  return {
    userId: draft.userId,
