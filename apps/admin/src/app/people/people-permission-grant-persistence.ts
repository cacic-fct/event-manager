  ...
  private async loadPermissionGrantTargets(): Promise<boolean> {
    try {
      const [events, majorEvents, eventGroups] = await Promise.all([
        ...
        firstValueFrom(
          this.permissionGrantsApi.listTargets(EventManagerPermissionGrantScope.MajorEvent, { take: 500 }),
        ),
        firstValueFrom(
          this.permissionGrantsApi.listTargets(EventManagerPermissionGrantScope.EventGroup, { take: 500 }),
        ),
      ]);
