...
export abstract class PeopleRecords extends PeopleState {
  ...
  async openLinkedDataDialog(person: Person): Promise<void> {
    const deleted = await firstValueFrom(
      this.dialog
        .open<PersonLinkedDataDialogComponent, PersonLinkedDataDialogData, boolean>(PersonLinkedDataDialogComponent, {
  ...
  private hasPersonDetails(person: Person): boolean {
    return (
      'secondaryEmails' in person || 'mergedIntoId' in person || 'externalRef' in person || 'lecturerProfile' in person
    );
  }
