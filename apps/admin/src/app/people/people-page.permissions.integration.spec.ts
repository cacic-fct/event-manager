...
function buttonByText(element: HTMLElement, text: string, options: { exact?: boolean } = {}): HTMLButtonElement | null {
  return (
    [...element.querySelectorAll('button')].find((button) => {
      const buttonText = button.textContent?.replace(/\s+/g, ' ').trim() ?? '';
      return options.exact
        ? buttonText.endsWith(text) && buttonText !== `auto_awesome ${text} do preset`
