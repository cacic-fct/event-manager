...
export abstract class PeopleState {
  ...
  readonly filteredPermissionGrantTargetOptions = computed(() => {
    ...
    return options
      .filter((target) => normalizeSearchText(`${target.label} ${target.description ?? ''}`).includes(query))
      .slice(0, 40);
  });
  ...
  readonly permissionGrantForm = this.formBuilder.nonNullable.group({
    ...
    category: this.formBuilder.nonNullable.control(
      parsePermission(Permission.Event.Read).resource,
      Validators.required,
    ),
  ...
  constructor() {
    ...
    this.permissionGrantForm.controls.scope.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((scope) => {
