...
function buildEntry(args: AuditLogDialogStoryArgs, index: number): AuditLogEntry {
  ...
  const operation =
    index === 0
      ...
      : faker.helpers.arrayElement<AuditLogOperation>([
          'UPDATE',
          'CREATE',
          'IMPORT',
          'APPROVE',
  ...
  return {
    ...
    actorEmail:
      actorType === 'USER'
        ? faker.internet.email({ firstName: actorName.split(' ')[0] }).toLocaleLowerCase('pt-BR')
        : null,
    actorType,
...
const scenarioByEntityType: Record<AuditLogEntityType, AuditLogScenario> = {
    ...
    changes: () => [
      {
        ...
        beforeValue: '20/06/2026',
        afterValue: '24/06/2026',
      },
  ...
  PLACE_PRESET: {
    ...
    changes: () => [
      ...
      {
        field: 'mergedSourceId',
        label: 'Origem unificada',
        beforeValue: 'place-old',
        afterValue: 'place-preset-story',
      },
    ],
  },
  PERMISSION_GRANT: {
  ...
  EVENT_SUBSCRIPTION: {
    ...
    changes: () => [
      ...
      {
        field: 'receiptValidatedAt',
        label: 'Comprovante validado em',
        beforeValue: null,
        afterValue: '26/06/2026 09:15',
      },
    ],
  },
  ...
  MAJOR_EVENT_SUBSCRIPTION: {
    ...
    changes: () => [
      { field: 'status', label: 'Situação', beforeValue: 'Em análise', afterValue: 'Rejeitada por conflito' },
      {
        ...
        label: 'Justificativa',
        beforeValue: null,
        afterValue: 'Conflito com outra atividade confirmada.',
      },
    ],
  },
  EVENT_ATTENDANCE: {
  ...
  EVENT_FORM: {
    entityId: 'event-form-story',
    label: 'formulário',
    primaryOperation: 'UPDATE',
    summary: 'Formulário atualizado.',
    permission: 'event-form#update',
    actorTypes: ['USER'],
    eventId: 'event-story',
    majorEventId: 'major-event-story',
    eventGroupId: 'event-group-story',
    changes: () => [
      { field: 'name', label: 'Nome', beforeValue: 'Pesquisa inicial', afterValue: 'Pesquisa de camiseta' },
      { field: 'publicationState', label: 'Publicação', beforeValue: 'Rascunho', afterValue: 'Publicado' },
    ],
  },
  EVENT_FORM_LINK: {
    entityId: 'event-form-link-story',
    label: 'vínculo de formulário',
    primaryOperation: 'UPDATE',
    summary: 'Vínculo de formulário atualizado.',
    permission: 'event-form#update',
    actorTypes: ['USER'],
    eventId: 'event-story',
    majorEventId: 'major-event-story',
  ...
  EVENT_FORM_RESPONSE: {
    entityId: 'event-form-response-story',
    label: 'resposta de formulário',
    primaryOperation: 'CREATE',
    summary: 'Resposta de formulário enviada.',
    permission: 'event-form#results',
    actorTypes: ['USER'],
    eventId: 'event-story',
    majorEventId: 'major-event-story',
    eventGroupId: null,
  ...
  CERTIFICATE_CONFIG: {
    ...
    changes: () => [
      { field: 'minimumAttendanceMinutes', label: 'Presença mínima', beforeValue: '60', afterValue: '75' },
      {
        field: 'issuedTo',
        label: 'Emitido para',
        beforeValue: 'Participantes',
        afterValue: 'Participantes e palestrantes',
      },
    ],
  },
  CERTIFICATE: {
