  ...
  return (
    changesByEntityType[entityType] ?? [
      { field: 'status', label: 'Situação', beforeValue: 'Pendente', afterValue: statusAfter },
      {
        ...
        label: 'Observação',
        beforeValue: null,
        afterValue: `${auditLogOperationLabel(operation)} registrada.`,
      },
    ]
  );
}
...
function snapshotForEntry(
  ...
  return {
    ...
    values: Object.fromEntries(
      changes.map((change) => [change.field, side === 'before' ? change.beforeValue : change.afterValue]),
    ),
  };
}

function contextForEntity(
  entityType: AuditLogEntityType,
  entityId: string,
): Pick<AuditLogExplorerEntry, 'eventId' | 'majorEventId' | 'eventGroupId'> {
  if (entityType === 'EVENT') {
