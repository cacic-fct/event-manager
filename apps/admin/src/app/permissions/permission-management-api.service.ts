...
export class PermissionManagementApiService {
  ...
  listRoles(includeArchived = false) {
    return this.graphql.request<{ permissionRoles: PermissionRole[] }>(`query PermissionRoles($includeArchived: Boolean) {
    ...
    }`, { includeArchived }).pipe(map((data) => data.permissionRoles));
  }

  listGroups(includeArchived = false) {
    return this.graphql.request<{ permissionGroups: PermissionGroup[] }>(`query PermissionGroups($includeArchived: Boolean) {
  ...
  searchPeople(query: string) {
    return this.graphql.request<{ people: Person[] }>(
  ...
  saveRole(input: PermissionRoleSaveInput) {
    return this.graphql.request<{ savePermissionRole: PermissionRole }>(
  ...
  saveGroup(input: PermissionGroupSaveInput) {
    return this.graphql.request<{ savePermissionGroup: PermissionGroup }>(
  ...
  archiveRole(id: string) {
    return this.graphql.request<{ archivePermissionRole: boolean }>(
  ...
  archiveGroup(id: string) {
    return this.graphql.request<{ archivePermissionGroup: boolean }>(
