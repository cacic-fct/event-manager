...
@Component({
    ...
    @if (pending.dirty()) {
      <aside class="unsaved-bar" [class.unsaved-bar--blocked]="pending.blockedNavigation()">
        ...
        <div>
          ...
          <button mat-flat-button type="button" [disabled]="pending.saving()" (click)="pending.save()">
            <mat-icon>save</mat-icon>{{ pending.saving() ? 'Salvando…' : 'Salvar alterações' }}
          </button>
        </div>
      </aside>
    }
  ...
  styles: `
    .unsaved-bar {
      position: fixed;
      z-index: 1100;
      inset-inline: max(1rem, calc((100vw - 72rem) / 2));
      bottom: 1rem;
      display: flex;
      align-items: center;
    ...
    .unsaved-bar--blocked {
      ...
      animation: blocked 700ms cubic-bezier(0.16, 1, 0.3, 1);
    }
    @keyframes blocked {
      30% {
