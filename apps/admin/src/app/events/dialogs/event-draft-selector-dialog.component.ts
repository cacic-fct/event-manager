...
export type EventDraftSelectorResult = { kind: 'original' } | { kind: 'draft'; draft: EventDraft };
        ...
        @for (draft of data.drafts; track draft.id) {
          <button mat-list-item type="button" (click)="selectDraft(draft)">
            ...
            <span matListItemLine>
              Editado em {{ draft.updatedAt | date: 'short' }} por
              {{ draft.updatedByName || draft.createdByName || 'usuário não identificado' }}
            </span>
            <span matListItemLine>
              Criado em {{ draft.createdAt | date: 'short' }} por
              {{ draft.createdByName || 'usuário não identificado' }}
            </span>
          </button>
        }
      </mat-action-list>
...
export class EventDraftSelectorDialogComponent {
  ...
  private readonly dialogRef =
    inject<MatDialogRef<EventDraftSelectorDialogComponent, EventDraftSelectorResult>>(MatDialogRef);
