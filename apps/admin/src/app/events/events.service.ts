...
import { CloneAssetDialogComponent, CloneAssetDialogResult } from './dialogs/clone-asset-dialog.component';
import {
...
export class EventsService {
  ...
  private async resolveDraftSelection(
    ...
    if (
      drafts.length === 0 ||
      (eventItem.publicationState !== 'PUBLISHED' && eventItem.publicationState !== 'SCHEDULED')
    ) {
      return null;
    }
  ...
  private showDraftRelationWarning(): void {
    this.snackbar.open('Rascunhos salvam apenas os campos do evento. Edite vínculos na versão publicada.', 'Fechar', {
      duration: 4000,
    });
  }
  ...
  private shouldSaveSeparateDraft(eventItem: Event, action: CreationPublicationAction): boolean {
    return (
      action === 'DRAFT' && (eventItem.publicationState === 'PUBLISHED' || eventItem.publicationState === 'SCHEDULED')
    );
  }
