...
describe('SportsIdentityProtectionService', () => {
  ...
  it('normalizes deterministic lookup hashes but randomizes authenticated ciphertext', () => {
    ...
    const first = service.protect(
      SportsIdentityType.EMAIL,
      '  Maria@Example.COM ',
    );
    const second = service.protect(
      SportsIdentityType.EMAIL,
      'maria@example.com',
    );
    ...
    expect(service.reveal(SportsIdentityType.EMAIL, first.encryptedValue)).toBe(
      'maria@example.com',
    );
    expect(
      service.matches(
  ...
  it('normalizes phone and identity-document variants before protecting them', () => {
    ...
    const document = service.protect(
      SportsIdentityType.IDENTITY_DOCUMENT,
      'abc-12.34',
    );

    expect(service.reveal(SportsIdentityType.PHONE, phone.encryptedValue)).toBe(
      '5511999991234',
    );
  ...
  it('rejects malformed, tampered, and semantically invalid values without leaking details', () => {
    const service = createService('test-secret');
    const protectedIdentity = service.protect(
      SportsIdentityType.EMAIL,
      'maria@example.com',
    );
    ...
    expect(() =>
      service.reveal(SportsIdentityType.EMAIL, tampered),
    ).toThrow(BadRequestException);
    expect(() =>
      service.reveal(SportsIdentityType.EMAIL, 'not-ciphertext'),
    ).toThrow(BadRequestException);
    expect(() => service.protect(SportsIdentityType.EMAIL, 'invalid')).toThrow(
      BadRequestException,
    );
    expect(() => service.protect(SportsIdentityType.PHONE, '123')).toThrow(
      BadRequestException,
    );
  });
  ...
  it('uses constant-time-safe length handling for untrusted lookup hashes', () => {
    ...
    expect(
      service.matches(SportsIdentityType.EMAIL, 'maria@example.com', 'short'),
    ).toBe(false);
  });
