...
describe('SportsPlayerApplicationRealtimeService', () => {
  ...
  it('publishes an opaque application invalidation only to its applicant', async () => {
    ...
    expect(realtime.publish).toHaveBeenCalledWith('sports-applications-person:person-1', {
  ...
  it('publishes payment changes to the person and admin scopes and invalidates routing', async () => {
    ...
    prisma.sportsPlayerApplication.findMany.mockResolvedValue([{ id: 'application-1', status: 'ACTIVE' }]);

    await service.publishPaymentChanged('subscription-1', 'PAYMENT_APPROVED');
  ...
  it('does not make a committed mutation appear failed when publication infrastructure fails', async () => {
    prisma.sportsPlayerApplication.findUnique.mockRejectedValue(new Error('database temporarily unavailable'));

    await expect(service.publishApplicationChanged('application-1', 'REVIEWED')).resolves.toBeUndefined();
  });
});
