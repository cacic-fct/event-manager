...
describe('PrizeDrawRealtimeController', () => {
  ...
  it.each([
    ...
    'authorizes and scopes the %s stream while preserving heartbeat data',
    async (_label, id, scopeType, method, target) => {
      ...
      const events = await firstValueFrom(controller[method](id, { user } as never, 'cursor-4').pipe(toArray()));
