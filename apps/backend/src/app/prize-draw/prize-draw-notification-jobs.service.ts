...
export class PrizeDrawNotificationJobsService implements OnModuleInit {
  ...
  async deliverWinner(spinId: string): Promise<void> {
    ...
    await this.prisma.prizeDrawSpin.updateMany({
      where: { id: spinId, notificationStatus: 'PENDING', undoneAt: null },
  ...
  async undoSpin(spinId: string): Promise<void> {
    ...
    const undoTransactionId = `prize-draw-undone:${spin.drawId}:${spin.sequence}`;
    const delivered = await this.notifications.notifyPrizeDrawUndone({
      ...this.notificationInput(spin),
    ...
    await this.prisma.prizeDrawSpin.update({
      ...
      data: {
        ...
        undoNotificationStatus: delivered ? 'SENT' : 'FAILED',
      },
    });
    await this.queue.add(
      ...
      { spinId },
      {
        ...
        delay: delivered ? UNDO_NOTIFICATION_RETENTION_MS : 5_000,
        attempts: 8,
        backoff: { type: 'exponential', delay: 5_000 },
        removeOnComplete: true,
        removeOnFail: true,
  ...
  private notificationRecord(spinId: string) {
    return this.prisma.prizeDrawSpin.findUnique({
      where: { id: spinId },
      select: {
        id: true,
        drawId: true,
