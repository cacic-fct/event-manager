import { PersonLinkedDataSummary, PersonLinkedResource, PersonLinkedResourcePage } from '@cacic-fct/shared-data-types';
...
export async function buildPersonLinkedResourcePage(
  ...
  const page = await buildRequestedLinkedResourcePage(
    prisma,
    personId,
    definition.type,
    pagination.skip,
    ...
    case 'EVENT_RELATION':
      return buildSegmentedPage(skip, take, [
        ...
        },
        {
          ...
          find: (segment) =>
            prisma.eventAttendanceCollector
              .findMany({
                where: { personId },
                include: { event: { select: { id: true, name: true, startDate: true } } },
                orderBy: { createdAt: 'desc' },
      ...
      return buildSegmentedPage(skip, take, [
        {
          ...
          find: async (segment) =>
            person.mergedInto && segment.skip === 0 && segment.take > 0
              ? [
                  {
