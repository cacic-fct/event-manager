...
export class PermissionManagementService {
  ...
  async saveGroup(input: PermissionGroupSaveInput, actor: AuthenticatedUser): Promise<PermissionGroup> {
    ...
    const existing = input.id
      ? await this.prisma.eventManagerPermissionGroup.findFirst({
    ...
    await this.runPermissionGraphTransaction(async (tx) => {
      ...
      await tx.eventManagerPermissionGroupMember.updateMany({
        ...
        data: {
          archivedAt: new Date(),
          archivedReason: EventManagerPermissionArchiveReason.MANUAL,
          updatedById: actorId,
        },
      });
      if (members.length) {
      ...
      const after = await tx.eventManagerPermissionGroup.findUniqueOrThrow({
        where: { id: groupId },
        select: groupSelect,
      });
      await this.auditLog.record(
  ...
  async archiveRole(id: string, actor: AuthenticatedUser): Promise<boolean> {
    ...
    await this.runPermissionGraphTransaction(async (tx) => {
      ...
      await tx.eventManagerRoleAssignment.updateMany({
        ...
        data: {
          archivedAt: now,
          archivedReason: EventManagerPermissionArchiveReason.ROLE_ARCHIVED,
  ...
  async archiveGroup(id: string, actor: AuthenticatedUser): Promise<boolean> {
    const group = await this.prisma.eventManagerPermissionGroup.findFirst({
    ...
    await this.runPermissionGraphTransaction(async (tx) => {
      ...
      await tx.eventManagerPermissionGroupMember.updateMany({
        ...
        data: {
          archivedAt: now,
          archivedReason: EventManagerPermissionArchiveReason.GROUP_ARCHIVED,
          updatedById: actor.sub,
        },
      });
      await this.auditLog.record(
  ...
  private normalizeAssignments(assignments: PermissionRoleAssignmentInput[]) {
    ...
    return assignments.map((assignment) => {
      ...
      if (assignment.scopes.length === 0)
        throw new BadRequestException('Cada atribuição precisa de ao menos um escopo.');
  ...
  private async assertGroupDelegation(actor: AuthenticatedUser, group: GroupRecord): Promise<void> {
    ...
    for (const role of this.mapRoles(roles, false)) {
      ...
      if (outsideCeiling.length)
        throw new ForbiddenException(`Você não pode delegar permissões que não possui: ${outsideCeiling.join(', ')}.`);
  ...
  private normalizeScopeTarget(
    ...
    const targetCount =
      Number(Boolean(normalized.eventId)) +
      Number(Boolean(normalized.majorEventId)) +
      Number(Boolean(normalized.eventGroupId));
    ...
    if (scope === EventManagerPermissionScope.EVENT && !normalized.eventId)
      throw new BadRequestException('Selecione o evento.');
    if (scope === EventManagerPermissionScope.MAJOR_EVENT && !normalized.majorEventId)
      throw new BadRequestException('Selecione o grande evento.');
    if (scope === EventManagerPermissionScope.EVENT_GROUP && !normalized.eventGroupId)
      throw new BadRequestException('Selecione o grupo de eventos.');
    return normalized;
  }
  ...
  private async resolveInputEffectivePermissions(
    ...
    const roles = await prisma.eventManagerRole.findMany({
      ...
      select: {
        ...
        systemKey: true,
        permissions: { select: { permission: true } },
        parentLinks: { where: { archivedAt: null }, select: { parentRoleId: true } },
      },
    });
  ...
  private async assertRoleInheritance(
    ...
    for (const link of links)
      parentsByChild.set(link.childRoleId, [...(parentsByChild.get(link.childRoleId) ?? []), link.parentRoleId]);
  ...
  private async assertAssignments(
    ...
    const [peopleCount, groupCount] = await Promise.all([
      ...
      groupIds.length
        ? prisma.eventManagerPermissionGroup.count({ where: { id: { in: groupIds }, archivedAt: null } })
        : 0,
    ]);
    if (peopleCount !== personIds.length || groupCount !== groupIds.length)
      throw new BadRequestException('Uma pessoa ou grupo atribuído não existe mais.');

    const eventIds = [
      ...new Set(
        assignments.flatMap((assignment) =>
          assignment.scopes.flatMap((scope) => (scope.eventId ? [scope.eventId] : [])),
        ),
      ),
    ];
    const majorEventIds = [
      ...new Set(
        assignments.flatMap((assignment) =>
          assignment.scopes.flatMap((scope) => (scope.majorEventId ? [scope.majorEventId] : [])),
        ),
      ),
    ];
    const eventGroupIds = [
      ...new Set(
        assignments.flatMap((assignment) =>
          assignment.scopes.flatMap((scope) => (scope.eventGroupId ? [scope.eventGroupId] : [])),
        ),
      ),
    ];
    ...
    if (
      ...
      majorEventCount !== majorEventIds.length ||
      eventGroupCount !== eventGroupIds.length
    ) {
    ...
    for (const assignment of assignments) {
      for (const scope of assignment.scopes) {
        const incompatible = permissions.filter(
          (permission) => !isPermissionGrantScopeCompatible(permission, scope.scope),
        );
        if (incompatible.length)
          throw new BadRequestException(`O escopo ${scope.scope} não é compatível com: ${incompatible.join(', ')}.`);
      }
  ...
  private async assertNoRedundantScopes(
    ...
    const groups = await prisma.eventGroup.findMany({
      where: {
        id: { in: scopes.flatMap((scope) => (scope.eventGroupId ? [scope.eventGroupId] : [])) },
        deletedAt: null,
      },
    ...
    for (let leftIndex = 0; leftIndex < scopes.length; leftIndex += 1) {
      for (let rightIndex = leftIndex + 1; rightIndex < scopes.length; rightIndex += 1) {
        ...
        if (major && child) {
          const childMajor = child.eventId
            ? eventById.get(child.eventId)?.majorEventId
            : child.eventGroupId
              ? groupById.get(child.eventGroupId)?.majorEventId
              : null;
          if (childMajor === major)
            throw new BadRequestException('Um grande evento já cobre este grupo ou evento filho no mesmo período.');
        }
  ...
  private validityOverlaps(
    ...
    const start = (scope: typeof left) =>
      Math.max(assignment.validFrom?.getTime() ?? -Infinity, scope.validFrom?.getTime() ?? -Infinity);
    const end = (scope: typeof left) =>
      Math.min(assignment.validUntil?.getTime() ?? Infinity, scope.validUntil?.getTime() ?? Infinity);
    return start(left) < end(right) && start(right) < end(left);
  }

  private async assertDelegation(
  ...
  private async assertDescendantDelegation(
    actor: AuthenticatedUser,
    roleId: string,
    permissions: Permission[],
  ): Promise<void> {
  ...
  private async runPermissionGraphTransaction<T>(operation: (tx: Prisma.TransactionClient) => Promise<T>): Promise<T> {
  ...
  private async assertGroupWriteStillValid(
    ...
    const transactionPrisma = tx as unknown as PrismaService;
    const current = existing
      ? await tx.eventManagerPermissionGroup.findFirst({
  ...
  private mapRoles(records: RoleRecord[], includeArchived: boolean): PermissionRole[] {
    ...
    const inherited = (role: RoleRecord, visiting = new Set<string>()): Set<string> => {
      ...
      for (const link of role.parentLinks) {
        ...
        if (!parent) continue;
        const definition = parent.systemKey
          ? EVENT_MANAGER_SYSTEM_ROLE_BY_KEY.get(parent.systemKey as EventManagerSystemRoleKey)
          : null;
        (definition?.permissions ?? parent.permissions.map((item) => item.permission)).forEach((permission) =>
    ...
    const mapped: PermissionRole[] = records.map((role) => {
      ...
      const assignments = role.assignments
        ...
        .map((assignment) => ({
          ...
          scopes: assignment.scopes
            .filter((scope) => includeArchived || !scope.archivedAt)
            .map((scope) => ({
              id: scope.id,
              scope: scope.scope,
              eventId: scope.eventId,
      ...
      return {
        ...
        permissions: [
          ...expandHardPermissionDependencies(
            definition?.permissions ?? role.permissions.map((item) => item.permission as Permission),
          ),
        ],
        ...
        groupPeopleCount: new Set(
          assignments.flatMap((assignment) =>
            assignment.groupId ? (groupMemberIdsByGroupId.get(assignment.groupId) ?? []) : [],
          ),
        ).size,
    ...
    if (!mapped.some((role) => role.systemKey === syntheticSuperAdmin.key)) {
      mapped.unshift({
        id: 'keycloak:super-admin',
        systemKey: syntheticSuperAdmin.key,
        name: syntheticSuperAdmin.name,
        description: syntheticSuperAdmin.description,
        emoji: syntheticSuperAdmin.emoji,
        isSystem: true,
        isExternal: true,
        assignable: false,
        version: 1,
        permissions: [...syntheticSuperAdmin.permissions],
  ...
  private mapGroup(group: GroupRecord, includeArchived: boolean): PermissionGroup {
    return {
      ...
      members: group.members
        .filter((member) => includeArchived || !member.archivedAt)
        .map((member) => ({
          id: member.id,
          person: {
            id: member.person.id,
