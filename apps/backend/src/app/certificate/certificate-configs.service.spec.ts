...
describe('CertificateConfigsService', () => {
  describe('payment tier settings', () => {
    function setup() {
      const prisma = createPrisma({
        certificateTemplate: { findFirst: jest.fn().mockResolvedValue({ id: 'template-1' }) },
        certificateConfig: {
          findFirst: jest.fn().mockResolvedValue(null),
          create: jest.fn().mockResolvedValue(createConfigRecord()),
        },
      });
      const priceTier = { findMany: jest.fn().mockResolvedValue([{ name: 'Aluno' }, { name: 'Professor' }]) };
      const service = new CertificateConfigsService(
        { ...prisma, priceTier } as never,
