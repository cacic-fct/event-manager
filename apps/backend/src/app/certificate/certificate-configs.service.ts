...
export class CertificateConfigsService {
  ...
  async updateConfig(configId: string, input: CertificateConfigUpdateInput): Promise<CertificateConfig> {
    ...
    const unchangedPaymentTiers =
      requestedPaymentTiers.length === (existingConfig.paymentTiers?.length ?? 0) &&
      requestedPaymentTiers.every((tier) => existingConfig.paymentTiers?.includes(tier));
    const paymentTiers =
      samePaymentTierTarget && unchangedPaymentTiers
    ...
    const data: Prisma.CertificateConfigUpdateInput = {
      ...
      ...(input.shouldAutofillSecondPage === undefined
        ? changedStandaloneMode
          ? { shouldAutofillSecondPage: mergedScope === CertificateScope.OTHER ? false : true }
          : {}
