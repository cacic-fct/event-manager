...
for (const appName of ['public', 'admin']) {
  ...
  for (const { name, code } of handlers) {
    test(`${appName} ${name} delegates a late response-writer rejection once`, async () => {
      ...
      let rejectWriter;
      const writer = new Promise((_, reject) => {
        rejectWriter = reject;
      });
      ...
      const context = {
        ...
        browserDistFolder: '/test/browser',
        applyCspToHtmlResponse: async (response) => response,
        publicCspPolicy: {},
        adminCspPolicy: {},
        addTurnstileSiteKeyMeta: (value) => value,
        isHtmlResponse: () => true,
        angularApp: { handle: async () => html },
      ...
      const errors = [];
      let delegated;
      const delegation = new Promise((resolve) => {
        delegated = resolve;
      });
      const run = context.module.exports({ path: '/index.html' }, {}, (error) => {
        errors.push(error);
        delegated();
      });
      await started;
